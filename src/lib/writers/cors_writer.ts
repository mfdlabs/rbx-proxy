...
export default abstract class CorsWriter {
  ...
  private static _initialize(): void {
    if (this._initialized && !corsEnvironment.singleton.corsRulesReloadOnRequest) return;

    this._initialized = true;

    const corsFile = path.join(corsEnvironment.singleton.corsRulesBaseDirectory, corsEnvironment.singleton.corsRulesFileName);
